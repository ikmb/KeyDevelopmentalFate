...
//sort_pred_degree**********************************
public void sort_pred_degree(ArrayList<Gene> g) {
	    if (g ==null || g.size()==0){
	      return;
	    }
	    this.GeneSet = g;
	    number = g.size();
	    quicksort_pred_degree(0, number - 1);
}
//quicksort_pred_degree**********************************
private void quicksort_pred_degree(int low, int high) {
	int i = low, j = high;
	int pivot = GeneSet.get(low + (high-low)/2).get_degree_pred();
	while (i <= j) {
		while (GeneSet.get(i).get_degree_pred() < pivot) {
	        i++;
	     }
	    while (GeneSet.get(j).get_degree_pred() > pivot) {
	        j--;
	     }
	     if (i <= j) {
	        exchange(i, j);
	...
	if (low < j)
		quicksort_pred_degree(low, j);
	if (i < high)
	    quicksort_pred_degree(i, high);
} 
